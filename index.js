...
/**
 * Функція `filterStudentsByGrade` фільтрує студентів за рівнем оцінки та виводить
 * інформацію про них.
 *
 *  grade - Рівень оцінки для фільтрації.
 */
function filterStudentsByGrade(students, grade) {
  // Виведемо в консоль критичну помилку з текстом "Якщо ви бачите це повідомлення,
  //то завдання 1 виконано не правильно"
  ...
  // За допомогою перебору масиву виведемо повідомлення
  //для відстеження роботи програми з іменем кожного студента, який має необхідну оцінку
  console.error(
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  // Якщо arr не є масивом або maxLength не є числом, виведеме в консоль критичну
  //помилоку з текстом: "Перший аргумент має бути масивом, другий аргумент має бути числом!".
  // Повертаємо null, що свідчить про неможливість обробки вхідних даних.
  // Перевіряємо, чи довжина масиву перевищує максимальну допустиму довжину.
  // Якщо довжина масиву перевищує максимальну,
  //виводимо попередження про потенційну проблеми з текстом:
  //"Увага! Довжина масиву перевищує максимально допустиму!".
  // Якщо довжина масиву не перевищує максимальну, виведеме в консоль
  //корисне повідомлення з текстом: "Довжина масиву не перевищує максимально допустиму.".
...
function compareArrays(arr1, arr2) {
  ...
  // Оскільки напряму порівняти масиви за допомогою оператора === не можно,
  //перетворимо ії в json і порівняємо
  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  arr1 = arr1.sort((num1, num2) => num1 - num2);
  arr2 = arr2.sort((num1, num2) => num1 - num2);
  ...
  if (typeof size === "number") {
    ...
    for (let i = 0; i < size; i++) {
      ...
      for (let j = 0; j < size; j++) {
        line[j]=(Math.floor(Math.random() * 10));
      }
	  arr[i]=line;
    }
	console.table(arr);
...
for ( const student of students){
	//console.log (student);
	console.info(`Студент: ${student.name}`)
...
function validateUserInput(userInput) {
  // Перевіремо умову наявності імені користувача, 
  //якщо воно відсутнє виводимо "Помилка: ім'я користувача відсутнє!"
  // Перевіремо умову наявності паролю, 
  //якщо він відсутній виводимо "Помилка: пароль відсутній!"
  // Перевірка довжини паролю чи менше вона ніж 8,
  //якщо ні виводимо повідомлення про потенційну 
  //проблему з текстом "Попередження: пароль має бути довшим за 8 символів!"
  ...
  //console.assert (userInput.password.length>8,"Попередження: пароль має бути довшим за 8 символів!" )
...
function countVowelsAndConsonants(word) {
  ...
  // Перевіряємо, чи є літера в нашому рядку з голосними, якщо так,
  // то виводимио лічильник голосних, інакше приголосних
  // Скидаємо лічильники
let vowels="aeiou";
